...
public:
      ...
      : myData()
   {
       ...
       myData.myHead->next = myData.myHead->prev = myData.myHead; //add
       myData.myHead->myVal = Ty(); //add
	   myData.mySize = right.myData.mySize;

	   auto it1 = myData.myHead;
	   auto it2 = right.begin(); //modify
	   
	   for (unsigned int a = 0; a < right.myData.mySize; a++, it2++) { //modify
		   it1 = it1->next;
		   //it2 = it2->next;  delete
           insert(end(), *it2);
		   //*it1 = *it2;  delete
   ...
   {	   
	   ...
	   if (where.ptr == myData.myHead || myData.mySize == 0)
		   temp->myVal = val;
	   else {
		   auto itlast = temp;
		   auto itcopy = temp->prev;
		   for (; itlast != where.ptr; itlast = itlast->prev, itcopy = itcopy->prev)
			   itlast->myVal = itcopy->myVal;
		   where.ptr->myVal = val;
	   }
	   myData.mySize++;
   }
