...
public:
   ...
   list& operator=( const list &right )
   {
      if( this != &right )
      {
          ...
          nodePtr templeft = myData.myHead;
          nodePtr tempright = right.myData.myHead;
          for (int a = 0; a < right.size(); a++) {
              templeft = templeft->next;
              tempright = tempright->next;
              templeft->myVal = tempright->myVal;
          }
      }
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    ...
    ListNode< Ty > nodeleft = left.myData.myHead;
    ListNode< Ty > noderight = right.myData.myHead;
    for (int a = 0; a < left.myData.mySize; a++) {
        nodeleft = nodeleft->next;
        noderight = noderight->next;
        if (nodeleft->myVal != noderight->myVal)
            return false;
    }
