...
#include "Vector - 1083321 - hw5.h" // include definition of class vector
...
bool HugeInteger< T >::operator<( const HugeInteger< T > &right ) const
{
    ...
    for (int a = integer.size()-1; a >= 0; a--) {
        if (integer[a] < right.integer[a])
            return true;
        else if (integer[a] > right.integer[a])
            return false;
    }
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   for (unsigned int a = 0; a < difference.integer.size(); a++)
       temp[a] = it1[a];
   ...
   for (int a = difference.integer.size() - 1; (*(it1 + a) == 0 && a >= 0); a--)
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   typename vector< T >::iterator it1 = quotient.integer.begin();

   HugeInteger buffer(integer.size());
   HugeInteger remainder(*this);
   typename vector< T >::iterator it_buffer = buffer.integer.begin();
   typename vector< T >::iterator it_remainder = remainder.integer.begin();
   typename vector< T >::iterator it2 = op2.integer.begin();
   unsigned int n = integer.size() - op2.integer.size();
   unsigned int quotient_size = integer.size() - op2.integer.size();

   for (unsigned int a = n; a < op2.integer.size() + n; a++)
       *(it_buffer + a) = *(it2 + a - n);
